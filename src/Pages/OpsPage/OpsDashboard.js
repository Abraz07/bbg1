...
import { LogOut, RefreshCw, Database, AlertCircle, FileText, Shield, Calendar } from 'lucide-react';
    ...
    const progressBar = (value, status) => {
        ...
        return (
            <div style={{ width: '100%', height: 6, borderRadius: 999, background: '#e5e7eb', overflow: 'hidden' }}>
                <div style={{ width: `${value}%`, height: '100%', background: barColor, transition: 'width 0.3s ease' }} />
            </div>
        );
    };
        ...
        <div className="dash">
            <div className={`side ${sidebarOpen ? 'open' : ''}`}>
                <div className="side-head">
                    ...
                    <div><h2>RW Tool</h2><p>Operations Panel</p></div>
                </div>
            ...
            <div className="main">
                ...
                <div style={{ marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                        <button style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', color: '#333', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
                            <Calendar style={{ width: 16, height: 16 }} />
                            Schedule Sync
                        </button>
                        <button style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 8, border: 'none', background: '#0473EA', color: '#fff', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
                ...
                <div style={{ marginBottom: 24, borderBottom: '2px solid #e5e7eb' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                        {[
                            ...
                            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', border: 'none', background: 'transparent', color: activeTab === tab.id ? '#0473EA' : '#6b7280', cursor: 'pointer', fontWeight: activeTab === tab.id ? 600 : 500, fontSize: 14, borderBottom: activeTab === tab.id ? '3px solid #0473EA' : '3px solid transparent', marginBottom: '-2px', transition: 'all 0.2s' }}>
                                {tab.icon}
                                {tab.label}
                            </button>
                        ))}
                ...
                <div style={{ marginBottom: 24, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                    {['Today', 'Last 7 days', 'Last 30 days'].map((filter) => (
                        <button key={filter} onClick={() => setActiveFilter(filter)} style={{ padding: '8px 16px', borderRadius: 8, border: activeFilter === filter ? '2px solid #0473EA' : '1px solid #e5e7eb', background: activeFilter === filter ? '#eff6ff' : '#fff', color: activeFilter === filter ? '#0473EA' : '#6b7280', cursor: 'pointer', fontWeight: activeFilter === filter ? 600 : 500, fontSize: 14 }}>
                            {filter}
                        </button>
                    ))}
                    <select value={selectedOrganization} onChange={(e) => setSelectedOrganization(e.target.value)} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', color: '#333', cursor: 'pointer', fontSize: 14, fontWeight: 500 }}>
                        <option>All Organizations</option>
                        <option>Finance</option>
                        <option>Risk Management</option>
                        <option>Operations</option>
                    </select>
                    <input type="text" placeholder="Search reports or sources..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', fontSize: 14, flex: 1, minWidth: 200 }} />
                </div>
                ...
                {activeTab === 'monitoring' && (
                    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12 }}>
                        <div style={{ padding: '20px', borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <p style={{ margin: '4px 0 0', fontSize: 14, color: '#6b7280' }}>Monitor connected storage sources and detect new reports</p>
                        </div>
                        <div style={{ padding: '20px' }}>
                            {storageConnections.map((conn) => (
                                <div key={conn.id} style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                                        ...
                                        <div>
                                            ...
                                            <p style={{ margin: 0, fontSize: 13, color: '#6b7280' }}>{conn.organization} • {conn.reports} reports • Last sync: {conn.lastSync}</p>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                                        <span style={{ padding: '6px 14px', borderRadius: 16, fontSize: 12, fontWeight: 600, background: conn.status === 'active' ? '#d1fae5' : '#fef3c7', color: conn.status === 'active' ? '#065f46' : '#92400e' }}>
                                            {conn.status === 'active' ? 'Active' : 'Syncing'}
                                        </span>
                                        <button style={{ padding: '8px 16px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', color: '#333', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                                            Check for Updates
                                        </button>
                                    </div>
                                </div>
                ...
                {activeTab === 'sync' && (
                    <div>
                        {failedOperations.length > 0 && (
                            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '20px', marginBottom: 24 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                                    <p style={{ margin: 0, fontSize: 15, fontWeight: 600, color: '#991b1b' }}>{failedOperations.length} failed operation</p>
                                    <button style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #dc2626', background: '#fff', color: '#dc2626', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Retry all failed</button>
                                </div>
                                {failedOperations.map((op) => (
                                    <div key={op.id} style={{ background: '#fff', border: '1px solid #fecaca', borderRadius: 8, padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
                                        ...
                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                            ...
                                            <button style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', color: '#333', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Retry</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                                ...
                                {syncJobs.map((job) => (
                                    <div key={job.id} style={{ marginBottom: 20 }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                                            <div>
                                                <p style={{ margin: '0 0 4px', fontSize: 15, fontWeight: 600, color: '#111' }}>{job.source} – {job.organization}</p>
                                                <p style={{ margin: 0, fontSize: 13, color: '#6b7280' }}>{job.files} files · {job.time}</p>
                                            </div>
                                            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                                {getStatusBadge(job.status)}
                                                {job.status === 'in-progress' && <button style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', color: '#333', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Cancel</button>}
                                                {job.status === 'failed' && <button style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', color: '#333', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Retry</button>}
                                            </div>
                                        </div>
                ...
                {activeTab === 'validation' && (
                    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12 }}>
                        <div style={{ padding: '20px', borderBottom: '1px solid #e5e7eb' }}>
                            ...
                            <p style={{ margin: '4px 0 0', fontSize: 14, color: '#6b7280' }}>Validate file integrity, format, and metadata before publishing</p>
                        </div>
                                    ...
                                    {pendingValidation.map((report) => (
                                        <tr key={report.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                                            ...
                                            <td style={{ padding: '16px 20px' }}>
                                                <span style={{ padding: '4px 10px', borderRadius: 12, fontSize: 12, fontWeight: 600, background: report.integrity === 'Pass' ? '#d1fae5' : '#fef3c7', color: report.integrity === 'Pass' ? '#065f46' : '#92400e' }}>
                                                    {report.integrity}
                                                </span>
                                            </td>
                                ...
                                <tbody>
                                    {publishedReports.map((report) => (
                                        <tr key={report.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                                            ...
                                            <td style={{ padding: '16px 20px' }}>
                                                <span style={{ padding: '4px 10px', borderRadius: 12, fontSize: 12, fontWeight: 600, background: report.notifications === 'Sent' ? '#d1fae5' : '#fef3c7', color: report.notifications === 'Sent' ? '#065f46' : '#92400e' }}>
